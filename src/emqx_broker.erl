...
-define(SERVERSTATUS,[<<"server_status">>, <<"server_delivered_status">>]).
%% Guards
        ...
        {ok, Msg1 = #message{topic = Topic}} ->
            case {element(7, Msg), element(4, Msg)} of
               ...
               {_, 'emqx_sys_mon'} ->
                ...
                _ ->
                    voifinity_message_datetime_change(Msg,Topic)
            end;
...
%%------------------------------------------------------------------------------
voifinity_message_datetime_change(Msg,Topic) ->
    DecodeMsg = jsx:decode(element(8, Msg)),
    DecodedMsg = element(2, hd(DecodeMsg)),
    UtcTime = erlang:system_time(milli_seconds),
    Messagetype = proplists:get_value(<<"message_type">>,DecodedMsg),
    ScheduledStatus = proplists:get_value(<<"scheduledstatus">>, DecodedMsg),
    ScheduledmsgProceed = proplists:get_value(<<"msgscheduled">>, DecodedMsg),
    case {lists:member(proplists:get_value(<<"message_type">>,DecodedMsg),?SERVERSTATUS) 
          , ScheduledStatus
          , ScheduledmsgProceed } of
      {true, _, _} ->
          Delivery = route(aggre(emqx_router:match_routes(Topic)), delivery(Msg)),
          Delivery#delivery.results;

      {false, undefined, _} ->
	 %io:format("updating time to UTC for User Message\n"),
         Message1 = lists:keyreplace(<<"timesort">>, 1, DecodedMsg, {<<"timesort">>, UtcTime}),
         Message2 = lists:keyreplace(<<"status">>, 1, Message1, {<<"status">>, <<"delivered">>}),
         Out = jsx:encode([{<<"data">>, Message2}]),
         Result = erlang:setelement(8, Msg, Out),
         Delivery = route(aggre(emqx_router:match_routes(Topic)), delivery(Result)),
         Delivery#delivery.results;
      ...
      {false, <<"true">>, <<"success">>} ->
          Message1 = lists:keyreplace(<<"status">>, 1, DecodedMsg, {<<"status">>, <<"delivered">>}),
          Out = jsx:encode([{<<"data">>, Message1}]),
          Result = erlang:setelement(8, Msg, Out),
          Delivery = route(aggre(emqx_router:match_routes(Topic)), delivery(Msg)),
          Delivery#delivery.results;
